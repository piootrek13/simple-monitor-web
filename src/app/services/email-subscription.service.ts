...
export class EmailSubscriptionService extends HttpService  {
  ...
  getSubsByGroup(groupId: number): Observable<EmailSubscription[]>{
    return this.http.get<EmailSubscription[]>("http://"+this.host+":8080/subscription/?groupId="+groupId, this.getOptions());
  }
  ...
  put(email: EmailSubscription): Observable<EmailSubscription[]>{
    return this.http.put<EmailSubscription[]>("http://"+this.host+":8080/subscription", email, this.getOptions());
  }
  delete(emailId: number): Observable<EmailSubscription[]>{
    return this.http.delete<EmailSubscription[]>("http://"+this.host+":8080/subscription/"+emailId, this.getOptions());
  }
}
